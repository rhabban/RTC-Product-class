...
class Product extends ProductCore {
    ...
    public function __construct($id_product = null, $full = false, $id_lang = null, $id_shop = null, Context $context = null)
    {
        ...
        //$this->quantity = $this->getExterneQuantity();*/
    }
                                            ...
                                            $id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0)
    {
        ...
        {
            $sql = new DbQuery();
            $sql->select('product_shop.`price`, product_shop.`ecotax`, p.reference');
            $sql->from('product', 'p');
            ...
            if (is_array($res) && count($res))
                foreach ($res as $row)
                {
                    ...
                    if($externeDatas = self::getExternePrice($reference)) {
                        $price = $externeDatas['price'];
                    }
                    else
                        $price = $row['price'];
        ...
        //var_dump(self::$_prices);
        return self::$_prices[$cache_id];
    }

    public static function getExternePrice($reference){
        $res = file_get_contents("http://biscuits-marguerite.com/RealTimeConnector/web/api/".$reference);
        $product = json_decode($res);
        $datas = array(
            "price" => $product->prix,
            "tva" => $product->taxe_rule
        );
        return $datas;
    }

    public function getExterneQuantity(){
        $res = file_get_contents("http://biscuits-marguerite.com/RealTimeConnector/web/api/".$this->reference);
        $product = json_decode($res);
